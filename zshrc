...
gcam() {
  ...
  local final_msg=$(echo "$diff" | claude -p "Generate a simple, straightforward git commit message for these changes. Be concise but specific about what was changed. Do not be overly detailed. Use proper sentences with correct capitalization and punctuation. Return only the commit message without any explanation or formatting." --output-format text | EDITOR=nvim vipe)
